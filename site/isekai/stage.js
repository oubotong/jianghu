...
class Stage{
 ...
 if(m&&m.id==='court-shoot'&&p>.15&&p<.85){const q=Math.min(1,(p-.15)/.7),x=hx+(ex-hx)*q,y=hy-115-Math.sin(q*Math.PI)*145; c.save();c.translate(x,y);c.rotate(q*7);c.fillStyle='#ef9947';c.strokeStyle='#4c2d26';c.lineWidth=3;c.beginPath();c.arc(0,0,13,0,Math.PI*2);c.fill();c.stroke();c.beginPath();c.moveTo(-13,0);c.lineTo(13,0);c.moveTo(0,-13);c.lineTo(0,13);c.stroke();c.restore();}
 if(m&&m.points&&p>.48){c.save();c.textAlign='center';c.font='900 40px "Microsoft YaHei",sans-serif';c.lineWidth=7;c.strokeStyle='#10252e';c.fillStyle='#ddfa8b';const label='＋'+m.points+' 分';c.strokeText(label,w*.5,h*.37);c.fillText(label,w*.5,h*.37);c.restore();}
