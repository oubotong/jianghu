...
function fresh(name='林小满',seed=Date.now(),style=null){const s={version:2,name:String(name).trim().slice(0,12)||'林小满',rng:seed>>>0,seed:seed>>>0,hp:MAX_HP,money:28,serial:0,deck:[],relics:['uniform'],clues:[],flags:{},route:[],index:0,phase:'event',battle:null,result:null,journal:[],completedEvents:0,wins:0,retries:1,ending:null};for(let i=0;i<4;i++){addCard(s,'strike');addCard(s,'guard');}addCard(s,'insight');addCard(s,'logic');if(D.styles[style]){s.style=style;D.styles[style].core.forEach((id,i)=>{s.deck[i*2].id=id;});}for(let chapter=0;chapter<6;chapter++){s.route.push({chapter,id:D.pools[chapter][0].id});for(const e of shuffle(s,D.pools[chapter].slice(1)).slice(0,4))s.route.push({chapter,id:e.id});s.route.push({chapter,id:'boss-'+chapter,boss:true});}return s;}
...
function attackExtra(s,v){const b=s.battle;return (!b.attackUsed&&has(s,'stamp')?2:0)+(v.fromBlock?Math.floor(b.block/2):0)+(v.chainBonus&&b.played.length>=2?v.chainBonus:0);}
function handCard(s,uid){const v=card(s,uid);if(!v||s.phase!=='battle')return v;const b=s.battle,notes=[];
 ...
 if(v.chainBonus){v.chainReady=b.played.length>=2;if(v.chainReady){v.damage+=v.chainBonus;}}v.bonusNote=notes.join('，');return v;
}
function cardEffects(v){
 ...
 if(v.chainBonus)add('energy','连招',v.chainReady?'已就绪':'待触发','',v.chainReady?'已计入每段'+v.chainBonus+'点加成':'本回合先出2张牌，每段伤害再加'+v.chainBonus+'点');
 if(v.detonate)add('poison','立刻触发洗脑',1,'次','按当前层数扣血，不减少层数');
 if(v.counterRatio)add('block','本回合反击',Math.round(v.counterRatio*100),'%','反弹挡下伤害的一半，无视护盾');
 if(v.retainBlock)add('block','下回合保留至多',v.retainBlock,'点剩余护盾');
 if(v.block)add('block','获得',v.block,'点护盾',v.retainBlock?'抵挡伤害，剩余部分可保留':'抵挡等量伤害，下回合开始清空');
...
function buildScores(s){return Object.entries(D.styles).map(([id,style])=>({id,score:style.pool.reduce((sum,cardId)=>sum+Math.min(2,s.deck.filter(c=>c.id===cardId).length)*(style.core.includes(cardId)?3:1),0)+(s.style===id ? .5 : 0)})).sort((a,b)=>b.score-a.score);}
function rewardOffers(s){
 const scores=buildScores(s),main=scores[0].score?scores[0].id:shuffle(s,Object.keys(D.styles))[0],other=shuffle(s,Object.keys(D.styles).filter(id=>id!==main))[0],used=new Set();
 function pick(pool,style){const missing=style?D.styles[style].core.filter(id=>!s.deck.some(c=>c.id===id)):[];const candidates=pool.filter(id=>!used.has(id));const bag=candidates.flatMap(id=>Array(missing.includes(id)?5:D.cards[id].heal&&s.hp<40?4:s.deck.some(c=>c.id===id&&!c.upgraded)?3:1).fill(id));const id=bag[Math.floor(random(s)*bag.length)];used.add(id);return id;}
 const a=pick(D.styles[main].pool,main),b=pick(D.styles[other].pool,other),c=pick(['logic','v50','rest','evidence','big','evolve','bomb']);
 return [{id:a,role:'match',style:main},{id:b,role:'explore',style:other},{id:c,role:'utility',style:null}];
}
function finish(s){const b=s.battle;if(s.hp<=0){s.hp=0;s.phase='defeat';return true;}if(b.hp>0)return false;s.wins++;const heal=Math.min(MAX_HP-s.hp,10+(has(s,'receipt')?4:0));s.hp+=heal;s.money+=18;s.journal.push({title:D.enemies[b.id].name,choice:'战胜对手',text:'靠手里的牌打赢了这一场。'});if(b.id==='goddess'){s.phase='ending';s.ending='truth';return true;}const offers=rewardOffers(s),options=offers.map(x=>x.id);s.result={title:'这一场，是你赢了',text:'新的一招会改变接下来的牌组。也可以跳过，让好牌更容易抽到。',rewards:['硬币 +18',...(heal?['生命 +'+heal]:[])],options,offers};s.phase='reward';return true;}
function play(s,uid){if(reason(s,uid))return null;const b=s.battle,v=card(s,uid),notes=[],motion={side:'hero',id:v.id,name:v.name+(v.upgraded?'＋':''),kind:v.kind,damage:0,heal:0};b.energy-=v.cost;b.hand.splice(b.hand.indexOf(uid),1);(v.exhaust?b.exhaust:b.discard).push(uid);
 ...
 if(v.damage){const extra=attackExtra(s,v);for(let i=0;i<(v.hits||1);i++)motion.damage+=hit(s,v.damage+extra,v.pierce||(!b.attackUsed&&has(s,'whistle')));}
 if(v.chainBonus&&b.played.length>=2)motion.proc='rush';
 if(v.detonate&&b.poison){const amount=hit(s,b.poison,true);motion.damage+=amount;motion.proc='chorus';notes.push('洗脑追加'+amount+'点伤害');}
 if(v.fromBlock&&b.block)motion.proc='bulwark';
 if(v.counterRatio)b.counterRatio=Math.max(b.counterRatio||0,v.counterRatio);
 if(v.retainBlock)b.retainBlock=Math.max(b.retainBlock||0,v.retainBlock);
...
function endTurn(s){if(s.phase!=='battle')return null;const b=s.battle,e=D.enemies[b.id],i=intent(s),motion={side:'enemy',id:b.id,name:i.name,kind:i.damage?'attack':'guard',damage:0,heal:0};if(b.poison){const hit=Math.min(b.hp,b.poison);b.hp-=hit;log(s,'洗脑副歌造成'+hit+'伤害。');b.poison--;if(finish(s)){motion.kind='skip';motion.name='副歌抢先收工';return motion;}}
 b.enemyBlock=i.block;const shield=Math.min(b.block,i.damage);b.block-=shield;motion.damage=Math.max(0,i.damage-shield-(i.damage>shield&&has(s,'headset')?1:0));const lost=Math.min(s.hp,motion.damage);s.hp=Math.max(0,s.hp-motion.damage);const notes=[i.damage?(lost?'你失去'+lost+'点生命':'你没有损失生命'):'本次未攻击'];if(shield)notes.push('护盾抵挡'+shield+'点伤害');if(i.damage>shield&&has(s,'headset'))notes.push('耳机减免1点伤害');if(i.block)notes.push('敌人护盾变为'+i.block+'点');if(b.id==='hakimi'&&i.damage){const healed=Math.min(b.maxHp-b.hp,3);b.hp+=healed;if(healed)notes.push('敌人恢复'+healed+'点生命');}if(b.counterRatio&&shield){motion.counterDamage=hit(s,Math.floor(shield*b.counterRatio),true);if(motion.counterDamage){motion.proc='bulwark';notes.push('护盾反击'+motion.counterDamage+'点伤害');}}log(s,i.name+'：'+notes.join('，')+'。');if(finish(s))return motion;
 b.discard.push(...b.hand);b.hand=[];b.round++;b.energy=ENERGY;b.block=Math.min(b.block,b.retainBlock||0);b.retainBlock=0;b.counterRatio=0;b.attackUsed=false;b.guardUsed=false;b.poisonUsed=false;b.played=[];b.weak=Math.max(0,b.weak-1);draw(s,DRAW+(has(s,'badge')?1:0));return motion;
}
function reward(s,id,mode='add'){if(s.phase!=='reward'||(id!=='skip'&&!s.result.options.includes(id))||!['add','upgrade'].includes(mode))return false;if(id!=='skip'){if(mode==='upgrade'){const target=s.deck.find(c=>c.id===id&&!c.upgraded);if(!target)return false;target.upgraded=true;s.journal.push({title:'战后强化',choice:D.cards[id].name+'＋',text:'将这次奖励用于强化已有卡牌，没有增加牌组张数。'});}else addCard(s,id);}s.phase='result';s.result={title:id==='skip'?'保持精简':'新招入手',text:id==='skip'?'你把奖励留给了下一位路过的人。':mode==='upgrade'?'「'+D.cards[id].name+'」已强化。':'「'+D.cards[id].name+'」已加入牌组。',rewards:[]};return advance(s);}
...
return {MAX_HP,ENERGY,DRAW,fresh,node,event,card,handCard,buildScores,rewardOffers,cardEffects,cardText,incoming,available,choose,upgrade,advance,startBattle,intent,reason,play,endTurn,reward,finale,retry,quit,valid};
});
